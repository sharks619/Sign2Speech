...
def setup(rank, world_size, args):
    ...
    dist.init_process_group(
        backend='nccl',
        init_method='tcp://163.152.177.36:12345',  # TCP 주소를 사용
        world_size=world_size,
        rank=rank
    )
    torch.cuda.set_device(rank)  # 각 프로세스가 자신의 GPU를 사용하도록 설정
...
def main_worker(rank, world_size, args):
    ...
    EPOCHS = 1
    start_epoch = 0
    cfg = setup(rank, world_size, args)
    cfg.freeze()
    train_loader, val_loader = build_data_loader(cfg, rank, world_size)
    print('rank: ', rank, 'train',train_loader.dataset.gloss)
    print('rank: ', rank, 'val',val_loader.dataset.gloss)
    loss_gls = nn.CTCLoss(blank=train_loader.dataset.sil_idx, zero_infinity=True).to(rank)
    model = SignModel(train_loader.dataset.vocab).to(rank)
    ...
    if cfg.RESUME:
        ...
        checkpoint = torch.load(cfg.RESUME, map_location=f'cuda:{rank}')
        ...
        logger.info(
            f"Loaded checkpoint from {cfg.RESUME}.  start_epoch: {start_epoch}, current_lr: {current_lr:.5f}, recoded WER: {best_wer:.3f}"
        )
    ...
    for epoch in range(start_epoch, EPOCHS):
        ...
        train_loader.sampler.set_epoch(epoch)  # DistributedSampler 사용 시 epoch 설정 필수
        ...
        for _iter in range(len(train_loader)):
            ...
            video_lengths = video_lengths.to(rank)
            glosses = glosses.to(rank)
            gloss_lengths = gloss_lengths.to(rank)
            ...
            if rank == 0 and (_iter + 1) % cfg.PERIODS.LOG_ITERS == 0:
                current_iter = epoch * len(train_loader) + _iter
                logger.info(
                    f"epoch: {epoch + 1}/{EPOCHS}, iter: {_iter + 1}/{len(train_loader)}, "
                    f"loss: {loss_meter.val:.3f} (avg: {loss_meter.avg:.3f}), "
                    f"iter_time: {iter_time_meter.val:.3f} (avg: {iter_time_meter.avg:.3f}), "
                    f"data_time: {data_time_meter.val:.3f} (avg: {data_time_meter.avg:.3f}), lr: {optimizer.param_groups[0]['lr']:.5f}"
                )
                ...
                writer.add_scalar("misc/lr", optimizer.param_groups[0]["lr"], current_iter)
        ...
        if rank == 0:
            ...
            logger.info(
                f"epoch: {epoch + 1}/{EPOCHS}, validation loss: {metrics['loss']:.3f}, validation WER: {metrics['wer']:.3f}"
            )
